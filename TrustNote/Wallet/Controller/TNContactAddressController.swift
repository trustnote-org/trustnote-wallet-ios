    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let deleteAction = UIContextualAction(style: .normal, title: nil) {[unowned self] (action, sourceView, completionHandler) in
            ...
            if TNLocalizationTool.shared.currentLanguage == "en" {
                titleText = "Do you want to delete the \"" + self.wallet.walletName + "\" address?"
            } else {
                titleText = "是否删除“" + self.wallet.walletName + "”常用地址？"
            }
            self.alertAction(self, titleText, message: nil, sureActionText: "Confirm".localized, cancelActionText: "Cancel".localized, isChange: true) {
                 self.deleteRow(indexPath: indexPath)
            }
            completionHandler(true)
        }
